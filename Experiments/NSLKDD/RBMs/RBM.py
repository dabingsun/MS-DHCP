...
#                 momentum 动量
                 ...
                 init_method = 'gauss',
                 CD_k=1,#一般就采用为1的方式
                 wPenalty = 0.0001,# 权重惩罚
                 cost_Fun = 'mse',#损失函数
                 relu_hidden = False, # hidden 是否relu
                 relu_visible = False # visible 是否relu
                 ):
        # check parameters
        ...
        if rbm_type.lower() == 'gbrbm':#转换字符串中所有大写字符为小写
            self.sig = 1.0;# Sigma
...
#            return errs
            weight,b1,b2 = sess.run(self.w),sess.run(self.hidden_bias),sess.run(self.visible_bias)
            return weight,b1,b2
